...
class SystemSemaphore:
    def __init__(self, name, limit, max_hold_time=600):
        self.name = name
        self.limit = limit
...
def compute_provability_training_data_remote(file_path, output_path, previous_output_path=None):
    try:
        ...
        #fin.close()
        fin = open('/home/mcwave/code/axiomatization/lean3/datasets/train_theorems_repo_mathlib3.pkl', 'rb')
            ...
            try:
                ...
                with SystemSemaphore('dojolock1', 1):
                    ...
                    try:
                        ...
                        dojo, state_0 = Dojo(theorem).__enter__() #dojo_enter(theorem)   # , copy_tree_on_enter=False)
                        entered = True
...
def main() -> int:
    ...
    print("Loading train theorems ...")
    fin = open('/home/mcwave/code/axiomatization/lean3/datasets/train_theorems_repo_mathlib3.pkl', 'rb')
    train_theorems = pickle.load(fin)
    fin.close()
    
    output_folder = '/home/mcwave/code/axiomatization/lean3/datasets/provability/rag/'
    ...
    for file_path, theorems in train_theorems.items():
        if theorems is not None and len(theorems) > 0 and 'src' in file_path:
            all_file_paths.append(file_path)
    random.shuffle(all_file_paths)
    print(len(all_file_paths), "file paths found")
